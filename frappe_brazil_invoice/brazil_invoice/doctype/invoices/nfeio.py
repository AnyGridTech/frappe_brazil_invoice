...
def calculate_taxes(invoice_doc, tax_template):
    ...
    try:
        ...
        frappe.log_error(
            "Could not connect to NFe.io API", "Tax Calculation Connection Error"
        )
        calculate_taxes_fallback(invoice_doc, tax_template)
...
def _build_api_payload(
    ...
    payload = {
        ...
        "tipoCliente": "J"
        if len(invoice_doc.client_id_number or "") == 14
        else "F",  # J=CNPJ, F=CPF
        "itens": items,
    }
...
def _update_invoice_with_tax_values(invoice_doc, tax_template, result):
    ...
    if tax_template.calculate_automatically_pis:
        pis_total = result.get("pis", {})
        if hasattr(invoice_doc, "pis_value"):
            invoice_doc.pis_value = float(pis_total.get("valor", 0))

    if tax_template.calculate_automatically_cofins:
        cofins_total = result.get("cofins", {})
        if hasattr(invoice_doc, "cofins_value"):
            invoice_doc.cofins_value = float(cofins_total.get("valor", 0))
