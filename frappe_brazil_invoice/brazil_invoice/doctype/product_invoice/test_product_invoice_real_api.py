...
def check_invoice_status_with_retries(invoice_id, invoice_name, max_retries=3, retry_delay=5, set_error_flag_callback=None):
    """
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if invoice_data.get("success"):
            ...
            if current_status == "Error":
                ...
                print(f"  - Items Count: {len(invoice.invoice_items_table) if invoice.invoice_items_table else 0}")
                print("="*70)
    ...
    # Max retries reached
    return {'success': True, 'status': 'Processing', 'note': 'Still processing after retries'}
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if pdf_result.get("success"):
                ...
                if attempt < max_retries:
                    print(f"  ⚠️ PDF retrieval failed: {error_msg}. Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                else:
            ...
            if attempt < max_retries:
                print(f"  ⚠️ Exception occurred: {str(e)}. Retrying in {retry_delay}s...")
                time.sleep(retry_delay)
            else:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if xml_result.get("success"):
                ...
                if attempt < max_retries:
                    print(f"  ⚠️ XML retrieval failed: {error_msg}. Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                else:
                    return {'success': False, 'error': error_msg}
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if cancel_result.get("success"):
                ...
                if attempt < max_retries:
                    print(f"  ⚠️ Cancellation failed: {error_msg}. Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                else:
                    return {'success': False, 'error': error_msg}
...
class TestProductInvoiceRealAPI(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        nfeio_configs = frappe.get_all(
            "NFeIO",
            fields=["name", "company_id", "is_test_config"]
        )
    ...
    def test_001_create_and_issue_invoice_1(self):
        ...
        # Ensure it's SP for internal operation
        if address_data["state"] != "SP":
            # If not SP, force it to be SP
            address_data = {
                "city": "São Paulo",
                "state": "SP",
                "cep": "01310-100",
        ...
        try:
            ...
            self.assertTrue(result.get("success"), f"API submission failed: {result.get('message')}")
            
            # Verify invoice_id is populated
            self.assertIsNotNone(invoice.invoice_id, "Invoice ID should be populated after API submission")
    ...
    def test_003_get_xml_for_invoice_1(self):
        ...
        # Get XML using helper function
        xml_result = get_xml_with_retries(invoice, max_retries=3, retry_delay=5)
        
        if not xml_result.get('success'):
            self.fail(f"XML retrieval failed: {xml_result.get('error')}")
        
        # Verify XML URL was returned
        self.assertIsNotNone(xml_result.get("xml_url"), "XML URL should be returned")

    def test_004_create_and_issue_invoice_2(self):
        """Create second invoice and issue it via real API"""
        frappe.set_user("Administrator")
        ...
        result = create_test_invoice_with_token(
            ...
            product_type="Inversor Solar",
            carrier=frappe.db.get_value("Carrier", {"fantasy_name": "Transportadora API Test"}, "name"),
            additional_information=f"Real API Test Invoice 2 - Interstate to {address_data['state']} - Will be cancelled",
            total_freight=totals_data["total_freight"],
            total_discount=totals_data["total_discount"],
            total_insurance=totals_data["total_insurance"],
            other_expenses=totals_data["other_expenses"],
        ...
        # Store for later tests and tracking
        frappe.flags.test_invoice_2 = invoice_name
        test_results["invoice_docnames"].append(invoice_name)

        # Fetch invoice
        invoice = frappe.get_doc("Product Invoice", invoice_name)
        
        print(f"\n✅ Invoice 2 created: {invoice.name}")
        print(f"  Client: {invoice.client_name}")
        print(f"  Status: {invoice.invoice_status}")
        print(f"  Total: R$ {invoice.total}")
        ...
        try:
            ...
            # IMPORTANT: Check if invoice has error status IMMEDIATELY
            # This must be done BEFORE any assertion that could fail
            if invoice.invoice_status == "Error":
                TestProductInvoiceRealAPI.invoice_2_error = True
                print("\n⚠️⚠️⚠️ CRITICAL: Invoice 2 has Error status")
                print("⚠️⚠️⚠️ Setting error flag - subsequent tests for Invoice 2 WILL be skipped")
                print(f"⚠️⚠️⚠️ Flag value: {TestProductInvoiceRealAPI.invoice_2_error}\n")
            else:
                TestProductInvoiceRealAPI.invoice_2_error = False
    ...
    def test_005_get_pdf_for_invoice_2(self):
        ...
        if TestProductInvoiceRealAPI.invoice_2_error:
            self.skipTest("Skipping: Invoice 2 has Error status")
        
        invoice_name = frappe.flags.get("test_invoice_2")
        self.assertIsNotNone(invoice_name, "Invoice 2 should exist from previous test")
    ...
    def test_007_cancel_invoice_2(self):
        """Cancel invoice 2 via API with retry logic"""
        frappe.set_user("Administrator")
        ...
        # Cancel using helper function
        cancel_result = cancel_invoice_with_retries(
            invoice,
            "Test cancellation for integration test",
            max_retries=3,
            retry_delay=5
        )
        
        if not cancel_result.get('success'):
            self.fail(f"Cancellation failed: {cancel_result.get('error')}")
        
        # Verify status changed to Unused
        self.assertEqual(
            invoice.invoice_status,
            "Unused",
            f"Invoice status should be Unused, got: {invoice.invoice_status}"
        )

    def test_008_verify_invoice_1_still_issued(self):
        """Verify invoice 1 is still in Processing or Issued status"""
        frappe.set_user("Administrator")
        ...
        print(f"\n✓ Verifying Invoice 1 status: {invoice.name}")
        print(f"  Status: {invoice.invoice_status}")
        print(f"  Invoice ID: {invoice.invoice_id}")
        
        # Verify invoice is still in Processing or Issued status (not Unused/Cancelled)
        self.assertIn(
            invoice.invoice_status, 
            ["Processing", "Issued"], 
            f"Invoice 1 should be Processing or Issued, got: {invoice.invoice_status}"
        )
        
        print(f"  ✅ Invoice 1 remains in {invoice.invoice_status} status as expected (not cancelled)")
